...
ChatBot.start = function(){
	$(document).ready(function() {
		...
		ChatBot.write("Hello! My name is Tel Aviv Cafebot. I am a chatbot designed to recommend cafes in Tel Aviv!","Cafebot");
		ChatBot.write("For a recommendation, please write: recommend me","Cafebot");
	});
};
...
ChatBot.initSpeechConfig = function(){
	if (ChatBot.speechEnabled ){
		...
		ChatBot.speechConfig.rate = 1.25;
		ChatBot.speechConfig.pitch = .75;
...
ChatBot.write = function(message,sender, emoji){
	...
	if (sender == "Cafebot" && ChatBot.speechEnabled){
...
ChatBot.handleServerError = function(errorThrown){
	...
	ChatBot.write("Sorry, there seems to be an error on the server. Let's talk later. " + actualError,"Cafebot");
